...
const PARLER_URL =
  "https://sarthakagrawal927--parler-tts-news-fastapi-app.modal.run/tts";
const SADTALKER_URL =
...
const PORTRAIT_PATH = resolve(
  REPO_ROOT,
  "web3d/public/assets/portraits/opm-z-city-mira.png"
);
...
async function generateTalkingHead(
  ...
  if (ctype.includes("application/json")) {
    ...
    if (!b64) {
      throw new Error(
        `SadTalker JSON had no video field. Keys: ${Object.keys(json).join(",")}`
      );
    }
...
async function main() {
  ...
  console.info(
    `  OK ${fmt(tts.ms)} | ${tts.bytes.length} bytes | meta keys: ${Object.keys(
      tts.meta
    ).join(",")}`
  );
  ...
  console.info(
    `Total wall-clock (TTS + SadTalker): ${fmt(tts.ms + vid.ms)}`
  );
}
