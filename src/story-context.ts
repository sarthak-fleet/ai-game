...
export function directorStoryBeatText(world: World): string | null {
  if (world.storyProgress?.phase === "nightfall_warning") {
    if (world.id === "ashment") return "Director beat: The Lantern Inn should be reached before the fog thickens.";
    if (world.id === "opm_z_city") return "Director beat: The Hero Association kiosk should be reached before the monster alert spreads.";
  ...
  if (world.storyProgress?.phase === "shadow_confrontation") {
    if (world.id === "ashment") return "Director beat: The lantern shadow is present; confronting it will force the night to resolve.";
    if (world.id === "opm_z_city") return "Director beat: The overpass challenger is in position; resolving the fight will clear this patrol loop.";
...
export function quietWorldRevealText(world: World): string {
  if (world.id === "ashment") return "The village has gone quiet enough for the bridge pattern to stand out.";
  if (world.id === "opm_z_city") return "Z-City has gone quiet enough for the overpass alert pattern to stand out.";
  const tension = world.tensions?.[0]?.title.toLowerCase() ?? "the pressure pattern";
...
export function phasePressureRevealText(world: World): string | null {
  if (world.storyProgress?.phase === "nightfall_warning") {
    if (world.id === "ashment") return "The Lantern Inn windows dim when the bridge whisper crosses the square.";
    if (world.id === "opm_z_city") return "The Hero Association kiosk lights flicker as the overpass alert spreads.";
  ...
  if (world.storyProgress?.phase === "shadow_confrontation") {
    if (world.id === "ashment") return "Lena's lantern throws one shadow too many across the inn floor.";
    if (world.id === "opm_z_city") return "The overpass challenger is close enough that witnesses start backing away.";
    const target = npcById(world, storyConfrontationTargetId(world));
...
export function planRevealText(world: World, planId: string, stage: number): string {
  ...
  if (world.id === "opm_z_city") {
    if (stage >= 3) return "The overpass alert is loud enough that civilians are clearing the street.";
