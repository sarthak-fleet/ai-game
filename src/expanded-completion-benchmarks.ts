...
export function expandedCompletionReport(rootDir = process.cwd()): ExpandedCompletionReport {
  const gates = [
    ...
    gate(rootDir, "generic_world_ingest", "Generic world ingest", [
      ...
      source("tests/world-ingest.test.ts", ["Skyfront Couriers: World Ingest Slice", "Clockwork Conservatory: World Ingest Slice", "world_origin_clue", "harbor_ring", "signal_mast", "guild_counter", "chain_bridge", "recover_route_token", "route_token", "a_false_pirate_alarm_threatens_the_harbor_route_plan", "recover_verdigris_key", "luminous_shell", "compiles a second non-anime genre", "keeps source-derived generic quest IDs playable", "uses source-derived story objectives", "Report to Guild Counter before pressure peaks", "Confront Vex", "World title is required.", "Duplicate world source entry", "invalid-source.json", "rejects malformed generic source", "Invalid world ingest source"]),
      source("tests/playtests/world-ingest.ts", ["Skyfront Couriers Playable Slice", "Agent loop controls", "Start", "running", "stopped", "3D travel", "At Harbor Ring", "Go Rookery Deck", "At Rookery Deck", "Pick up Route token", "Complete: Give Route token", "Recover Prism gear for Ivo", "Pick up Prism gear", "Recover Painted flag scrap for Nell", "Pick up Painted flag scrap", "Report to Guild Counter before pressure peaks", "Call Vex into the open with Nell watching.", "Skyfront Couriers Playable Slice route stabilized", "That matters in Skyfront Couriers Playable Slice", "Ask about world", "canvasPixelHash", "importInvalidSource", "World import failed: invalid_world_source", "One Punch Man Playable Slice"]),
    ]),
    gate(rootDir, "long_running_agents", "Long-running AI agents", [
      ...
      source("web/src/store/world.ts", ["applyServerTick", "commitTick", "lastSummary", "agentLoopStatus", "setAgentLoopStatus", "result.agentLoopStatus", "bubbles"]),
      source("web/src/templates/AppShell.tsx", ["AgentLoopPanel", "Agents"]),
      source("src/agent-loop.ts", ["checkpointEveryTicks", "maxTicks", "start()", "stop(", "restoreCheckpoint", "clearCheckpoints", "restoredCheckpoint", "agent_loop_step_in_progress", "initialCheckpoints", "maxCheckpoints", "slice(-maxCheckpoints)"]),
