...
export function expandedCompletionReport(rootDir = process.cwd()): ExpandedCompletionReport {
  const gates = [
    ...
    gate(rootDir, "generic_world_ingest", "Generic world ingest", [
      ...
      source("tests/playtests/world-ingest.ts", ["Skyfront Couriers Playable Slice", "3D travel", "At Harbor Ring", "Go Rookery Deck", "At Rookery Deck", "Pick up Route token", "Complete: Give Route token", "Recover Prism gear for Ivo", "Pick up Prism gear", "Recover Painted flag scrap for Nell", "Pick up Painted flag scrap", "Report to Guild Counter before pressure peaks", "Call Vex into the open with Nell watching.", "Skyfront Couriers Playable Slice route stabilized", "That matters in Skyfront Couriers Playable Slice", "Ask about world", "canvasPixelHash", "importInvalidSource", "World import failed: invalid_world_source", "One Punch Man Playable Slice"]),
    ]),
    gate(rootDir, "long_running_agents", "Long-running AI agents", [
      ...
      source("src/agent-loop.ts", ["checkpointEveryTicks", "maxTicks", "start()", "stop(", "restoreCheckpoint", "restoredCheckpoint", "agent_loop_step_in_progress", "initialCheckpoints", "maxCheckpoints", "slice(-maxCheckpoints)"]),
