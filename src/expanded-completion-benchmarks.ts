...
export function expandedCompletionReport(rootDir = process.cwd()): ExpandedCompletionReport {
  const gates = [
    gate(rootDir, "three_d_variant", "3D variant", [
      ...
      source("web/src/three/world-scene.ts", ["Raycaster", "onLocationSelect", "onNpcSelect", "onItemSelect", "onPropSelect", "onTargetHover", "onContextStatus", "webglcontextlost", "webglcontextrestored", "orbitCamera", "resetCamera", "TRAVEL_ANIMATION_MS", "previousActorPositions", "movingActors", "actorMotion", "animateTravel", "makeTargetHalo", "sceneAnimation", "makePathMesh", "makeLabelSprite", "makeLandmarkMesh", "makePropMesh", "makeAtmosphereMesh", "atmosphereKindFor", "locationPalette"]),
      source("web/src/organisms/ThreeWorld.tsx", ["3D travel", "3D target", "3D camera controls", "3D agent activity", "3D renderer status", "latestAutonomousActivity", "action.type === \"move\"", "Rotate camera right", "Reset camera", "onKeyDown", "keyboardDestinationFor", "hoverTarget", "movePlayerToward", "openDrawer", "pickup", "inspect", "ashbend:travel-to"]),
      ...
      source("tests/playtests/first-loop.ts", ["Slot Save", "page.reload", "importInvalidSave", "Restore failed: Save file is not valid JSON", "Slot Load", "three-host canvas", "At Herb Garden"]),
    ...
    gate(rootDir, "generic_world_ingest", "Generic world ingest", [
      ...
      source("tests/world-ingest.test.ts", ["invalid-source.json", "rejects malformed generic source", "Invalid anime ingest source"]),
