...
export function expandedCompletionReport(rootDir = process.cwd()): ExpandedCompletionReport {
  const gates = [
    gate(rootDir, "two_d_primary_with_three_d_shelf", "2D primary with 3D shelf", [
      ...
      source("web/src/three/world-scene.ts", ["Raycaster", "onLocationSelect", "onNpcSelect", "onItemSelect", "onPropSelect", "onTargetHover", "onContextStatus", "webglcontextlost", "webglcontextrestored", "orbitCamera", "zoomCamera", "cameraZoomPercent", "resetCamera", "TRAVEL_ANIMATION_MS", "previousActorPositions", "movingActors", "actorMotion", "animateTravel", "makeTargetHalo", "sceneAnimation", "makePathMesh", "makeTerrainGrid", "terrainNode", "terrain-grid", "makeRouteNode", "makeLabelSprite", "makeActorNameplate", "actor-nameplate", "makeLandmarkMesh", "reef_spire", "sonar_dish", "neon_sign", "rain_lamp", "evidence_board", "transformer_stack", "makePropMesh", "makeAtmosphereMesh", "atmosphereKindFor", "makeWeatherMesh", "weatherNode", "kind: \"rain\"", "bubbles", "clouds", "pollen", "locationPalette", "actorBodyShapeFor", "makeActorAccessoryMesh", "bodyShape", "accentColor", "npc.id !== world.player.characterId", "SceneMoodNode", "sceneMoodForClock", "applySceneMood", "timeOfDay", "PCFSoftShadowMap", "applyShadows"]),
      source("web/src/organisms/ThreeWorld.tsx", ["3D travel", "3D target", "3D camera controls", "3D camera zoom", "Zoom camera in", "Zoom camera out", "3D agent activity", "3D renderer status", "latestAutonomousActivity", "action.type === \"move\"", "Rotate camera right", "Reset camera", "onKeyDown", "keyboardDestinationFor", "sceneTargetForWorld", "activateSceneTarget", "event.key.toLowerCase() === \"e\"", "three-interact-button", "Interact with", "No scene target", "movePlayerToward", "openDrawer", "pickup", "inspect", "ashment:travel-to"]),
      source("web/src/three/scene-targets.ts", ["activeObjectives", "sceneTargetForWorld", "objectiveSceneTargetFor", "validSceneTarget", "keyboardTargetFor", "reachableLocations"]),
      ...
      source("tests/three-interactions.test.ts", ["prefers the local objective NPC", "prefers the active objective item", "keeps a valid hovered target"]),
      file("fixtures/saves/not-json.json"),
      source("tests/playtests/first-loop.ts", ["Slot Save", "page.reload", "phaser-host canvas", "Bring Pruning shears to Mira", "openInteractPanel", "Inspect", "Fresh soot outlines", "importInvalidSave", "Restore failed: Save file is not valid JSON", "Slot Load", "Complete: Give Pruning shears"]),
      source("tests/playtests/alive-village.ts", ["three-host canvas", "nonBlankCanvasPixels", "canvasPixelHash", "webglcontextlost", "webglcontextrestored", "3D renderer paused", "3D renderer restored", "choosePlayableCharacter", "Tomas", "travelStartHash", "Rotate camera right", "Zoom camera in", "Zoom camera out", "3D camera zoom", "Reset camera", "keyboard.press(\"d\")", "keyboard.press(\"a\")", "Go Village Square", "Go Herb Garden", "At Village Square", "At Herb Garden", "hoverThreeTarget", "Talk Mira", "Interact with Mira", "Focus", "HUD", "focus-mode", "three-overlay", "expectNoHorizontalOverlap", "dialogue-panel", "toHaveClass(/active/)", "Recoverable app error", "Dismiss", "expectWithinViewport", "expectNoVerticalOverlap"]),
      ...
      source("tests/three-world.test.ts", ["structureColor", "landmarks", "forge_tool_rack", "objectives[0]", "terrain", "gridColor", "weather", "kind: \"clouds\"", "kind: \"pollen\"", "kind: \"bubbles\"", "kind: \"rain\"", "moves the primary 3D objective beacon", "chosen character as the 3D player avatar", "bodyShape: \"broad\"", "bodyShape: \"caped\"", "bodyShape: \"mechanical\"", "changes 3D lighting mood with the world clock", "fogDensity", "atmosphere", "firefly", "dust", "source-derived artifact visuals", "source-derived location palettes", "conservatory-source.json", "abyssal-source.json", "noir-source.json", "reef_spire", "sonar_dish", "neon_sign", "rain_lamp", "evidence_board", "transformer_stack", "shape: \"gear\"", "shape: \"scrap\""]),
    ...
    gate(rootDir, "generic_world_ingest", "Generic world ingest", [
      ...
      source("src/anime-ingest.ts", ["locationPaletteFor", "locationLandmarksFor", "generatedPortraitDataUri", "data:image/svg+xml", "visualTags", "greenhouse|botanical", "cloud|sky|harbor", "sonar|hydrophone", "reef|coral", "neon|noir|\\brain\\b", "reef_spire", "sonar_dish", "neon_sign", "rain_lamp", "evidence_board", "transformer_stack"]),
      ...
      source("web/src/world-samples.ts", ["WORLD_SAMPLES", "Skyfront", "Clockwork", "Abyssal", "Neon Noir", "skyfront-source.json", "conservatory-source.json", "abyssal-source.json", "noir-source.json"]),
      source("web/src/organisms/AppHeader.tsx", ["Reviewed sample worlds", "handleSampleWorld", "WORLD_SAMPLES.map", "World source JSON", "Import", "World import failed"]),
      ...
      source("tests/world-ingest.test.ts", ["Skyfront Couriers: World Ingest Slice", "Clockwork Conservatory: World Ingest Slice", "Abyssal Salvage: World Ingest Slice", "Neon Nocturne: World Ingest Slice", "data:image/svg+xml", "generated portrait", "world_origin_clue", "harbor_ring", "signal_mast", "guild_counter", "chain_bridge", "recover_route_token", "route_token", "a_false_pirate_alarm_threatens_the_harbor_route_plan", "recover_verdigris_key", "luminous_shell", "reef_dome", "sonar_array", "recover_pearl_key", "coral_shell", "rain_market", "signal_booth", "recover_witness_badge", "desk_radio", "a_forged_confession_threatens_the_rain_market_case_plan", "compiles a second non-anime genre", "compiles an undersea salvage genre", "compiles a neon noir mystery genre", "covers a diverse set of non-anime world genres", "keeps source-derived generic quest IDs playable", "uses source-derived story objectives", "Report to Guild Counter before pressure peaks", "Confront Vex", "World title is required.", "Duplicate world source entry", "invalid-source.json", "rejects malformed generic source", "Invalid world ingest source"]),
      source("tests/playtests/world-ingest.ts", ["Skyfront Couriers Playable Slice", "Clockwork Conservatory Playable Slice", "Abyssal Salvage Playable Slice", "Agent loop controls", "Start", "running", "stopped", "0 autonomous ticks", "Autonomous agents waiting", "3D travel", "At Harbor Ring", "Go Rookery Deck", "At Rookery Deck", "Pick up Route token", "Complete: Give Route token", "Recover Prism gear for Ivo", "Pick up Prism gear", "Pick up Painted flag scrap", "Report to Guild Counter before pressure peaks", "Call Vex into the open with Nell watching.", "Skyfront Couriers Playable Slice route stabilized", "Recover Verdigris key for Eda", "Pick up Verdigris key", "Recover Pearl key for Neri", "At Reef Dome", "At Sonar Array", "Pick up Pearl key", "Complete: Give Pearl key", "verifyAbyssalLiveLoop", "abyssalLiveLoopBeforeHash", "generic imported world live agent loop should visibly update the 3D scene", "Checkpoints\\s*\\d+", "Restore latest", "Restored checkpoint: world tick \\d+", "restored Abyssal checkpoint should keep 3D canvas nonblank", "09-abyssal-checkpoint-restore.png", "08-abyssal-live-loop.png", "quickSaveImportedAbyssal", "Quick saved: Abyssal Salvage Playable Slice", "quickLoadImportedAbyssal", "Quick loaded: Abyssal Salvage Playable Slice", "quick-loaded imported Abyssal 3D canvas should render nonblank pixels", "verifyImportedPackageReview", "Package healthy.", "Story package review", "Abyssal Salvage: World Ingest Slice", "No structural issues found.", "11-abyssal-package-review.png", "10-abyssal-quick-load.png", "verifyMobileImportedAbyssal", "Abyssal mobile 3D canvas should render nonblank pixels", "expectNoVerticalOverlap", "08-abyssal-mobile.png", "07-abyssal-source.png", "clickButton(page, \"Close\")", "expectGeneratedPortrait", "generated portrait", "data:image/svg+xml", "naturalWidth", "canvasPixelHash", "importInvalidSource", "World import failed: invalid_world_source", "One Punch Man Playable Slice", "LIVE_LOOP_INTERVAL_MS", "opmLiveLoopBeforeHash", "imported world live agent loop should visibly update the 3D scene", "autonomousTickCount", "08-opm-live-loop.png"]),
      source("tests/playtests/world-ingest.ts", ["NOIR", "Neon Nocturne Playable Slice", "Recover Witness badge for Reva", "Noir 3D canvas should render nonblank pixels", "noirStartHash", "not.toEqual(abyssalStartHash)", "completeNoirFirstQuest", "At Rain Market", "At Signal Booth", "Pick up Witness badge", "Complete: Give Witness badge", "Recover Signal lens for Milo", "Noir 3D canvas should change after evidence movement", "12-noir-source.png"]),
    ]),
    gate(rootDir, "long_running_agents", "Long-running AI agents", [
      ...
      source("web/src/templates/AppShell.tsx", ["AgentLoopPanel", "Agents", "drawerNpcId", "dialogue-mode"]),
      source("web/style.css", [".dialogue-mode > header", "max-height: min(560px, calc(100vh - 112px))", ".dialogue-mode .dialogue-panel", ".objective-tracker", "z-index: 45"]),
      ...
      source("src/agents.ts", ["refreshMoods", "directorPressure > 65", "emotion = \"anxious\"", "isNightTime"]),
      source("src/llm/proposer.ts", ["Faction:", "Description:", "Needs:", "Mood:", "Current intent:"]),
      ...
      source("tests/agent-state.test.ts", ["refreshMoods updates NPC emotion", "directorState!.pressure = 80", "emotion).toBe(\"anxious\")"]),
      ...
      source("tests/playtests/alive-village.ts", ["Agent loop controls", "toBeGreaterThanOrEqual(1)", "toBeGreaterThanOrEqual(6)", "Restore latest", "Restored checkpoint: world tick \\d+", "restoredCheckpointTick", "ticksBeforeRestore", "ticksBeforeLiveLoop", "liveLoopBeforeHash", "live agent loop should visibly update the 3D scene", "autonomousTickCount", "LIVE_LOOP_INTERVAL_MS", "3D agent activity", "Autonomous t\\d+", "Autonomous t(?:[7-9]|\\d{2,})", "Autonomous agents waiting", "running", "stopped"]),
    ]),
    ...
    gate(rootDir, "verification_surface", "Everything tested", [
      ...
      packageScript("bench:differentiation"),
      packageScript("bench:commercial"),
      ...
      packageScript("playtest:production"),
      packageScript("playtest:astro-landing"),
      packageScriptContains("playtest:all", ["playtest:first-loop", "playtest:basic-v0", "playtest:alive", "playtest:opm", "playtest:world-ingest", "playtest:production", "playtest:astro-landing"]),
      packageScriptContains("verify:readiness", ["typecheck", "lint", "test", "build", "budget:bundle", "bench:completion", "bench:expanded", "bench:differentiation", "bench:commercial", "playtest:all"]),
      file("scripts/copy-web-assets.mjs"),
      file("tests/completion-benchmarks.test.ts"),
      file("tests/expanded-completion-benchmarks.test.ts"),
      file("tests/competitive-differentiation.test.ts"),
      file("tests/commercial-readiness.test.ts"),
      file("tests/unreal-bridge.test.ts"),
      file("src/commercial-readiness.ts"),
      file("tests/playtests/production-build.ts"),
      file("tests/playtests/astro-landing.ts"),
      source("package.json", ["node scripts/copy-web-assets.mjs", "playtest:production", "serve:production", "WEB_ROOT=./dist/web node dist/server/server.js"]),
      source("tests/playtests/production-build.ts", ["WEB_ROOT", "dist/web", "dist/server/server.js", "REQUIRED_ASSETS", "/assets/cc0/russpuppy/open_tileset_16.png", "/assets/cutscenes/ashment_intro_square.mp4", "production 3D canvas should render nonblank pixels", "production agent step should visibly update 3D scene", "Reviewed sample worlds", "00-production-world-gallery.png", "Neon Noir imported.", "villageStartHash", "Neon Nocturne Playable Slice", "Recover Witness badge for Reva", "Talk Reva", "Interact with Reva", "production imported noir 3D canvas should render nonblank pixels", "production noir 3D canvas should visually differ from village", "Neon Nocturne: World Ingest Slice", "03-production-noir-import.png", "production mobile imported 3D canvas should render nonblank pixels", "01-production-3d.png", "02-production-mobile.png"]),
      ...
      source("tests/playtests/astro-landing.ts", ["Ashment | Interactive AI World", "playable 3D RPG simulation", "World Ingest", "Long-Running Agents", "Use W A S D to explore the 3D world", "Import structured world source JSON", "#three-ambient canvas", "landing Three.js background should render nonblank pixels", "bodyWidth", "viewportWidth", "01-desktop.png", "02-mobile.png"]),
      file("docs/ai-dungeon-differentiation.md"),
      file("src/competitive-differentiation.ts"),
      source("src/competitive-differentiation.ts", ["competitiveDifferentiationReport", "competitor_boundary", "positive_positioning", "avoid_clone_claims", "behavioral_proof", "activeSurfaceAvoids"]),
      source("docs/ai-dungeon-differentiation.md", ["AI Dungeon", "Last rechecked: 2026-05-22", "Story Cards", "Memory Banks", "Image Generator", "Do, Say, Story, or See", "AI World Simulator, a playable 3D world simulator", "Structured world compiler", "Long-running autonomous agents", "Playable objectives over open-ended prose drift", "Persistence and recoverability", "Do not claim \"infinite text adventure\"", "Browser-tested desktop and mobile playability", "Competitive Readiness Gate", "Neon Nocturne", "noir mystery world with distinct visual treatment", "Production-build browser smoke tests"]),
    ]),
  ];
