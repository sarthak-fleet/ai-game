...
export function expandedCompletionReport(rootDir = process.cwd()): ExpandedCompletionReport {
  const gates = [
    gate(rootDir, "three_d_variant", "3D variant", [
      ...
      source("web/src/organisms/ThreeWorld.tsx", ["3D travel", "3D target", "3D camera controls", "3D camera zoom", "Zoom camera in", "Zoom camera out", "3D agent activity", "3D renderer status", "latestAutonomousActivity", "action.type === \"move\"", "Rotate camera right", "Reset camera", "onKeyDown", "keyboardDestinationFor", "keyboardTargetFor", "validSceneTarget(world, hoverTarget)", "validSceneTarget(latestWorld, hoverTarget)", "function validSceneTarget", "activateSceneTarget", "event.key.toLowerCase() === \"e\"", "three-interact-button", "Interact with", "No scene target", "movePlayerToward", "openDrawer", "pickup", "inspect", "ashment:travel-to"]),
    ...
    gate(rootDir, "verification_surface", "Everything tested", [
      ...
      packageScriptContains("verify:readiness", ["typecheck", "lint", "test", "build", "budget:bundle", "bench:completion", "bench:expanded", "bench:differentiation", "playtest:all"]),
