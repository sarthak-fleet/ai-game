...
export function expandedCompletionReport(rootDir = process.cwd()): ExpandedCompletionReport {
  const gates = [
    ...
    gate(rootDir, "long_running_agents", "Long-running AI agents", [
      ...
      source("web/style.css", [".dialogue-mode > header", "max-height: min(560px, calc(100vh - 160px))", ".dialogue-mode .dialogue-panel"]),
      ...
      source("tests/playtests/alive-village.ts", ["Agent loop controls", "1 autonomous ticks", "6 autonomous ticks", "Restore latest", "Restored checkpoint: world tick 5", "20:00", "18:00", "liveLoopBeforeHash", "live agent loop should visibly update the 3D scene", "autonomousTickCount", "LIVE_LOOP_INTERVAL_MS", "3D agent activity", "Autonomous t1", "Autonomous t(?:[7-9]|\\d{2,})", "Autonomous agents waiting", "running", "stopped"]),
    ]),
    ...
    gate(rootDir, "verification_surface", "Everything tested", [
      ...
      source("tests/playtests/production-build.ts", ["WEB_ROOT", "dist/web", "dist/server/server.js", "REQUIRED_ASSETS", "/assets/cc0/russpuppy/open_tileset_16.png", "/assets/cutscenes/ashbend_intro_square.mp4", "production 3D canvas should render nonblank pixels", "production agent step should visibly update 3D scene", "villageStartHash", "Neon Nocturne Playable Slice", "Recover Witness badge for Reva", "production imported noir 3D canvas should render nonblank pixels", "production noir 3D canvas should visually differ from village", "Neon Nocturne: World Ingest Slice", "03-production-noir-import.png", "production mobile imported 3D canvas should render nonblank pixels", "01-production-3d.png", "02-production-mobile.png"]),
