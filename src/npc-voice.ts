...
const ARCHETYPE_EXAMPLES: Record<string, [string, string][]> = {
  ...
  scholar: [
    ["Free for a chat?", "Briefly. The librarian's spirit notes — they need stamping. Can it wait until Thursday? Friday's worse."],
    ["What do you think of all this?", "Seventeen precedents. None resolved cleanly. I would not want your position."],
    ["Do you know anything about the threat?", "Three hypotheses. The second one worries me most. Where did you last see it?"],
  ],
  blunt: [
  ...
  anxious: [
    ["Everything alright?", "Yes — no. Maybe. The east gate was unlocked this morning and nobody's explaining it."],
    ["Can I ask you something?", "Of course, just — keep your voice down? Someone was listening earlier, I'm almost certain."],
    ["Are you the right person to talk to?", "Probably not, but everyone else walked away. So. Yes. Ask me."],
  ],
  warm: [
    ["Good to see you.", "And you! Sit, I was just about to make tea. Terrible timing for a crisis, but here we are."],
...
function archetypeFromTraits(personality: string[], flaws: string[]): string {
  ...
  if (all.some((t) => t.includes("stoic") || t.includes("quiet") || t.includes("reserved") || t.includes("deadpan") || t.includes("flat")))
    return "stoic";
  if (all.some((t) => t.includes("scholar") || t.includes("analytic") || t.includes("intellectual") || t.includes("studious") || t.includes("serious")))
    return "scholar";
  if (all.some((t) => t.includes("blunt") || t.includes("harsh") || t.includes("rude") || t.includes("dismissive") || t.includes("impatient") || t.includes("sharp")))
    return "blunt";
  if (all.some((t) => t.includes("anxious") || t.includes("nervous") || t.includes("worr") || t.includes("paranoid") || t.includes("fearful")))
    return "anxious";
  if (all.some((t) => t.includes("warm") || t.includes("kind") || t.includes("empathetic") || t.includes("nurturing") || t.includes("earnest") || t.includes("brave") || t.includes("protective")))
    return "warm";
  if (all.some((t) => t.includes("direct") || t.includes("honest") || t.includes("pragmatic") || t.includes("loyal") || t.includes("casual")))
    return "direct";
  return "impatient";
}
...
export function voiceFingerprint(npc: Npc): string {
  ...
  if (all.some((t) => t.includes("deadpan") || t.includes("flat") || t.includes("stoic") || t.includes("quiet")))
  ...
  if (all.some((t) => t.includes("blunt") || t.includes("harsh") || t.includes("rude") || t.includes("dismissive")))
    cadenceTics.push("skips pleasantries entirely; answers what was asked, not what was meant");
  if (all.some((t) => t.includes("warm") || t.includes("kind") || t.includes("empathetic") || t.includes("earnest")))
...
function currentActivity(npc: Npc): string {
  ...
  if (intent) {
    switch (intent.kind) {
      case "confront": return "jaw set, looking for someone to confront";
      case "investigate": return "picking through details, distracted";
      case "hide": return "keeping close to the wall, scanning exits";
      case "gossip": return "leaning in, already mid-conversation";
      case "move": return "clearly about to leave";
      case "trade": return "weighing something in their hands";
      case "help": return "ready to act, watching for a signal";
      case "avoid": return "positioning themselves away from the crowd";
      case "escalate": return "coiled, voice lower than usual";
      default: break;
    }
  }
  // Fall back to role
  ...
  if (role.includes("guard") || role.includes("soldier") || role.includes("warrior")) return "watching the entrance";
  if (role.includes("innkeeper") || role.includes("bartender")) return "wiping down the counter";
  if (role.includes("scholar") || role.includes("scribe") || role.includes("librarian")) return "hunched over a ledger";
  if (role.includes("blacksmith") || role.includes("smith")) return "sharpening a blade";
  if (role.includes("healer") || role.includes("doctor") || role.includes("physician")) return "sorting supplies";
  if (role.includes("hero") || role.includes("adventurer") || role.includes("hunter")) return "keeping alert";
...
export function rightNowFor(world: World, npc: Npc): string {
  ...
  if (recentConfrontation) {
    ...
    if (negativeReflection) {
      ...
      const relLabel =
        rel <= -6 ? "freshly betrayed by you"
        : rel <= -3 ? "wary of you"
        : rel < 2 ? "reading you still"
        : rel < 5 ? "warm to you"
        : "trusts you";
      parts.push(relLabel);
    }
...
export function divergenceNudges(npc: Npc): string[] {
  ...
  if (all.some((t) => t.includes("blunt") || t.includes("direct") || t.includes("harsh") || t.includes("flat") || t.includes("deadpan")))
    add("DO NOT pad with apologies or hedge.");
  if (all.some((t) => t.includes("impatient") || t.includes("dismissive") || t.includes("sharp")))
    add("DO NOT pad with apologies or hedge.");
  if (all.some((t) => t.includes("scholar") || t.includes("analytic") || t.includes("serious") || t.includes("formal") || t.includes("intellectual")))
