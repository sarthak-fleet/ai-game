...
export function upsertAgentLoopCheckpoint(path: URL, checkpoint: AgentLoopCheckpoint, maxCheckpoints: number): void {
  const existing = readAgentLoopCheckpoints(path).filter((candidate) => candidate.tick !== checkpoint.tick);
...
function isCheckpoint(value: unknown): value is AgentLoopCheckpoint {
  ...
  return typeof checkpoint.tick === "number"
    && typeof checkpoint.capturedAt === "string"
    && Boolean(checkpoint.world && typeof checkpoint.world === "object" && typeof checkpoint.world.id === "string");
}
