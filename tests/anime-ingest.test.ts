...
import { type AnimeIngestSource, animeSourceToWorld, validateAnimeIngestSource } from "../src/anime-ingest.ts";
...
describe("anime ingest compiler", () => {
  test("turns a reviewed anime bible into a playable world", () => {
    ...
    expect(engine.state.locations.map((location) => location.id)).toEqual(["square", "forge", "garden", "inn", "bridge", "wood"]);
  ...
  test("rejects under-specified anime source drafts", () => {
    ...
    expect(validateAnimeIngestSource(draft)).toEqual(expect.arrayContaining([
      { path: "locations", message: "At least three locations are required for a playable slice." },
      { path: "characters", message: "At least three characters are required for starter quests." },
    ]));
