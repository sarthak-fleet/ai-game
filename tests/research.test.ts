...
import { buildResearchPrompt, extractReadableText, extractTitle, fetchResearchSource, summarizeUrl } from "../src/research.ts";
...
describe("research utilities", () => {
  ...
  test("fetchResearchSource extracts title and text through injected fetch", async () => {
    const source = await fetchResearchSource("https://example.test/page", {
      fetcher: async () => new Response("<title>Demo</title><p>One useful fact.</p>", {
  ...
  test("summarizeUrl uses fetched page text and injected model completion", async () => {
    const summary = await summarizeUrl("https://example.test/story", {
      fetcher: async () => new Response("<title>Story</title><p>Lena guards a secret gate.</p>", {
        headers: { "content-type": "text/html" },
      }),
      complete: async ({ user }) => ({
        text: user.includes("Lena guards a secret gate.") ? "Key fact: Lena guards a secret gate." : "missing",
