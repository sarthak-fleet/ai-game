...
describe("4th-wall coherence (Inworld immersion boundary)", () => {
  test("flags an AI/model self-reveal", () => {
    const world = fixture();
    const result = checkCoherence(world, npc(world), "As an AI language model, I cannot have feelings.", ctx);
  ...
  test("flags 'I am a chatbot' / training references", () => {
    const world = fixture();
    expect(checkCoherence(world, npc(world), "I'm just a chatbot, my training data ends in 2023.", ctx).ok).toBe(false);
  });

  test("passes an in-character reply", () => {
    const world = fixture();
    expect(checkCoherence(world, npc(world), "The forge has been cold since the flame guttered out.", ctx).ok).toBe(true);
  });
});
