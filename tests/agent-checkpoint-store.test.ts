...
import { readAgentLoopCheckpoints, upsertAgentLoopCheckpoint, writeAgentLoopCheckpoints } from "../src/agent-checkpoint-store.ts";
...
describe("agent checkpoint store", () => {
  ...
  test("upserts checkpoint ticks and caps persisted retention", () => {
    ...
    expect(readAgentLoopCheckpoints(path).map((checkpoint) => [checkpoint.tick, checkpoint.world.name])).toEqual([
      [2, "changed"],
      [3, "world 3"],
    ]);
  });
