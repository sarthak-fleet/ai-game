...
import { questDefeatTargetId, questObjectiveBlockText, questObjectiveMet } from "../src/quest-objectives.ts";
...
describe("quest objectives", () => {
  ...
  it("the engine refuses complete_quest before the task is done, with a human reason", () => {
    ...
    quest.status = "active";
    quest.acceptedBy = "player";
    const verdict = validateAction(world, { type: "complete_quest", actorId: "player", questId: "return_shears" });
    expect(verdict.ok).toBe(false);
    if (!verdict.ok) expect(verdict.reason).toContain("Pruning shears");
    world.items.find((item) => item.id === "shears")!.holderId = "player";
    expect(validateAction(world, { type: "complete_quest", actorId: "player", questId: "return_shears" }).ok).toBe(true);
  });
  ...
  it("prose-only quests stay uncheckable rather than blocked", () => {
    ...
    const verdict2 = validateAction(world, { type: "complete_quest", actorId: "player", questId: "dyn_vibes" });
    expect(verdict.ok).toBe(false);
