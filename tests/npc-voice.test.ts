...
describe("voiceFingerprint", () => {
  it("produces non-overlapping diction strings for two distinct trait sets", () => {
    const stoic = makeNpc({
      id: "stoic_warrior",
      traits: { personality: ["deadpan", "stoic"], values: ["duty"], flaws: ["underreacts to danger"] },
    });
...
describe("rightNowFor", () => {
  ...
  it("surfaces the current goal / intent kind as physical activity", () => {
    ...
    const npc = makeNpc({
      ...
      locationId: "square",
      role: "guard",
      plan: { currentIntent: { kind: "investigate", targetId: "square", reason: "anomaly", updatedTick: 1 } },
    });
    const line = rightNowFor(world, npc);
