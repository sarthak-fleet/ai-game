...
describe("confrontations move bodies", () => {
  it("a leaked secret leads to a cross-town confrontation with witnesses", async () => {
    ...
    holder.secrets = [{ id: "s1", text: "Mira hides the stolen moonmint shipment under the garden shed", risk: 80, knownBy: [] }];
    ...
    for (let index = 0; index < 8 && !confronted; index += 1) {
      ...
      confronted = summary.actions.some((entry) => entry.action.type === "confront" && entry.text.includes("confronts"));
    }
    ...
    expect(witness.memories.some((memory) => memory.text.includes("watched") && memory.meta?.visibility === "shared")).toBe(true);
  ...
  it("abandons the grudge if the target is gone", async () => {
    ...
    judge.ambitions = [
      ...(judge.ambitions ?? []),
      { id: "lena_confront_ghost_5", title: "Confront Ghost", kind: "reveal", priority: 84, status: "active" },
    ];
    await runTick(world, undefined, scripted);
    expect(judge.ambitions!.find((goal) => goal.id === "lena_confront_ghost_5")!.status).toBe("abandoned");
  });
});
