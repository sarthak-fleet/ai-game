...
describe("web3d director beats", () => {
  ...
  it("ignores ordinary actions", () => {
    useDirectorStore.getState().maybeTriggerFromSummary(
      summaryWith([{ action: { type: "talk", actorId: "mira", targetId: "player", text: "hi" }, text: "Mira talked." }]),
      world,
      world
    );
  ...
  it("prefers a villain plan stage advance and frames the villain", () => {
    const prev: World = { ...world, villainPlans: [{ id: "p", actorId: "pax", title: "Duel", objective: "o", stage: 1, hidden: true, pressure: 50 }] };
    const next: World = {
      ...world,
      villainPlans: [{ id: "p", actorId: "pax", title: "Duel", objective: "o", stage: 2, hidden: true, pressure: 60, nextTrigger: "Pax calls out a public challenge." }],
    };
