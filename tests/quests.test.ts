...
describe("quests", () => {
  test("player accepts, completes — relationship +2 both ways", () => {
    const world = fixture();
    const accept = applyAction(world, { type: "accept_quest", actorId: "player", questId: "return_shears" });
  ...
  test("fail_quest applies consequences", () => {
    ...
    const fail = applyAction(world, { type: "fail_quest", actorId: "player", questId: "return_shears" });
  ...
  test("only the accepter can complete", () => {
    const world = fixture();
    applyAction(world, { type: "accept_quest", actorId: "player", questId: "return_shears" });
    const result = applyAction(world, { type: "complete_quest", actorId: "tomas", questId: "return_shears" });
  ...
  test("cannot accept twice", () => {
    const world = fixture();
    applyAction(world, { type: "accept_quest", actorId: "player", questId: "return_shears" });
    const second = applyAction(world, { type: "accept_quest", actorId: "tomas", questId: "return_shears" });
  ...
  test("npc can offer a quest to player", () => {
    ...
    const offer = applyAction(world, { type: "offer_quest", actorId: "mira", targetId: "player", questId: "return_shears" });
