...
import { storyPackageFromWorld, validateStoryPackage, worldFromStoryPackage } from "../src/story-package.ts";
...
describe("story package shape", () => {
  ...
  test("validation catches importer-scale reference issues", () => {
    ...
    expect(validateStoryPackage(pkg)).toEqual(expect.arrayContaining([
      { path: `world.locations.${pkg.world.locations[0]!.id}`, message: `Duplicate id "${pkg.world.locations[0]!.id}" is not allowed.` },
      { path: "world.exits.square->missing_exit.to", message: "Exit target location must exist in the package world." },
      { path: `world.items.${pkg.world.items[0]!.id}.holderId`, message: "Item holder must be player or an NPC in the package world." },
      { path: `world.interactables.${pkg.world.interactables[0]!.id}.relatedQuestId`, message: "Interactable related quest must exist in the package quest list." },
      { path: `world.tensions.${pkg.world.tensions[0]!.id}.involvedIds`, message: `Tension involved id "missing_reference" must reference a known NPC, item, location, or player.` },
      { path: `world.villainPlans.${pkg.world.villainPlans[0]!.id}.actorId`, message: "Villain plan actor must exist in the package NPC list." },
    ]));
  });
});
