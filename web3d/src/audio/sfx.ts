...
function tone(frequency: number, duration: number, type: OscillatorType, peak: number, glideTo?: number): void {
...
function noise(duration: number, peak: number, filterFrequency: number, filterType: BiquadFilterType = "lowpass"): void {
  if (!enabled || !context || !master) return;
