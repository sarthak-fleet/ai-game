    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={(event) => event.stopPropagation()}>
        ...
        <p className="modal-hint">
          Paste a world-ingest source JSON (title, synopsis, locations, characters…) — the 3D city, characters, and quests
        ...
        <div className="modal-actions">
          <label className="modal-file">
            Load file
            <input type="file" accept=".json" onChange={(event) => loadFile(event.target.files?.[0])} hidden />
          </label>
          <button type="button" onClick={onClose}>
            Cancel
          </button>
          <button type="button" className="primary" disabled={importing || !draft.trim()} onClick={() => void submit()}>
