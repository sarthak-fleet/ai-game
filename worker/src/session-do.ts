...
export class GameSessionDO {
  ...
  private async ensureEngine(): Promise<{
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === '/api/worlds' && request.method === 'GET') {
      return json(200, {
        worlds: BUNDLED_WORLDS.map(({ id, name, blurb, kind, beta, showcase }) => ({
          id,
          name,
          blurb,
          kind,
    ...
    if (
      (path === '/api/import-world-source' || path === '/api/import-anime') &&
      request.method === 'POST'
    ) {
      if (this.rateLimited('replace_world')) return json(429, { error: 'rate_limited' });
      ...
      try {
        ...
        if (!source || typeof source !== 'object') {
          return json(400, {
            error: 'invalid_world_source',
            issues: [{ path: 'source', message: 'World ingest source is required.' }],
          });
        }
    ...
    if (path === '/api/agent-loop/status' && request.method === 'GET')
      return json(200, agentLoop.status());
    if (path === '/api/agent-loop/start' && request.method === 'POST')
      return json(200, agentLoop.start());
    if (path === '/api/agent-loop/stop' && request.method === 'POST')
      return json(200, agentLoop.stop());
    ...
    if (path === '/api/dialogue/choose' && request.method === 'POST') {
      ...
      if (typeof body?.npcId !== 'string' || typeof body?.optionId !== 'string')
        return json(400, { error: 'npcId and optionId required' });
      ...
      if (reply.action) {
        this.broadcast('tick', {
          summary: {
            ...
            actions: [
              {
                action: { type: reply.action.type, actorId: body.npcId, targetId: 'player' },
        ...
        this.ctx.waitUntil(
          (async () => {
            ...
            const onToken = (delta: string) => {
              tokensSent = true;
              void writer.write(
                this.encoder.encode(`event: token\ndata: ${JSON.stringify({ t: delta })}\n\n`)
              );
            };
            let result = await generateDialogueReply(engine.state, npcId, trimmed, {
              onToken,
              historyKey,
            });
            if (
              !result.ok &&
              !tokensSent &&
              !['unknown_npc', 'npc_defeated', 'npc_not_here'].includes(result.reason)
            ) {
              result = await generateDialogueReply(engine.state, npcId, trimmed, {
                onToken,
                historyKey,
              });
            }
            if (result.ok && result.action) {
              this.broadcast('tick', {
                summary: {
                  ...
                  actions: [
                    {
                      action: { type: result.action.type, actorId: npcId, targetId: 'player' },
    ...
    if (path === '/api/arc/event' && request.method === 'POST') {
      ...
      if (body?.kind === 'spar_won') {
              ...
              actions: [
                {
                  action: {
                    type: 'remember',
                    actorId: engine.state.arc?.mentorId ?? 'player',
                    text: 'The spar is won.',
                  },
        ...
        return json(200, {
          ok: true,
          arc: engine.state.arc ?? null,
          growth: engine.state.player.growth ?? null,
        });
      }
